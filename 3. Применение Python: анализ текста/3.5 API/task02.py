...
# и выполнить необходимые шаги, а именно зарегистрироваться, создать приложение, и получить пару идентификаторов Client Id и Client Secret.
...
# в то время как для записи имен на сайте используется кодировка UTF-8, что может привести к ошибке при попытке записать в файл имя с необычными символами.
...
client_id = "ffd301099ec4df121810"
client_secret = "d7226bd7081d50f1840b913a948f5fda"
...
with open("dataset_24476_4.txt") as f:
    for id in f:
        id = id.strip()
        request = requests.get(f"https://api.artsy.net/api/artists/{id}", headers=headers)
        response = json.loads(request.text)
        dict_artist[response["sortable_name"]] = response["birthday"]

dict_artist_sorted = sorted(dict_artist.items(), key=lambda x: (x[1], x[0]))

for artist in dict_artist_sorted:
    print(artist[0])
